...
export class Gauge extends Widget {
  ...
  private drawNeedle(
    ...
    if (this.pv?.value !== undefined) {
      ...
      valuePosition =
        360 - this.getValuePosition((limits.min + limits.max) / 2, limits);
    }
  ...
  private getValuePosition(v: number, limits: DisplayLimits) {
    ...
    if (this.logScale) {
      valuePosition =
        START_ANGLE -
        ((Math.log10(v) - Math.log10(limits.min)) /
          (Math.log10(limits.max) - Math.log10(limits.min))) *
          lengthInDegrees;
    } else {
      valuePosition =
        START_ANGLE -
        ((v - limits.min) / (limits.max - limits.min)) * lengthInDegrees;
    }
