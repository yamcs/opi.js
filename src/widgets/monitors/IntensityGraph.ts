...
import { BooleanProperty, ColorMapProperty, ColorProperty, FloatProperty, FontProperty, IntProperty, StringProperty } from '../../properties';
...
export class IntensityGraph extends Widget {
    ...
    draw(g: Graphics) {
        ...
        if (this.xAxisVisible) {
            xTitleHeight = g.measureText(this.xAxisAxisTitle, this.xAxisTitleFont).height;
        }

        let yTitleWidth = 0;
        if (this.yAxisVisible) {
            yTitleWidth = g.measureText(this.yAxisAxisTitle, this.yAxisTitleFont).height;
        }

        const xScale = new LinearScale(scale, this.xAxisScaleFont, this.xAxisMinimum,
            this.xAxisMaximum, false, this.xAxisMajorTickStepHint, this.xAxisAxisColor,
            this.xAxisShowMinorTicks, this.xAxisVisible);
        const xScaleMargin = xScale.calculateMargin(g, true);
        const yScale = new LinearScale(scale, this.yAxisScaleFont, this.yAxisMinimum,
            this.yAxisMaximum, false, this.yAxisMajorTickStepHint, this.yAxisAxisColor,
            this.yAxisShowMinorTicks, this.yAxisVisible);
        ...
        const yScaleWidth = yScale.drawVertical(g, area.x + yTitleWidth, area.y, yScaleHeight, true);
        ...
        const verticalLineX = Math.round(area.x + yTitleWidth + yScaleWidth) - (lineWidth / 2);
        const verticalLineY = Math.round(area.y + yScaleMargin) - (lineWidth / 2);
        g.strokePath({
            path: new Path(verticalLineX, verticalLineY)
                .lineTo(verticalLineX, verticalLineY + graphAreaHeight),
        ...
        g.strokePath({
            path: new Path(horizontalLineX, horizontalLineY)
                .lineTo(horizontalLineX + graphAreaWidth, horizontalLineY),
        ...
        if (this.showRamp) {
            const rampScale = new LinearScale(scale, this.font, min, max, false,
                50 * scale, this.alarmSensitiveForegroundColor, false, true);
            const rampY = area.y + yScale.margin;
            const rampFullHeight = graphAreaHeight;
            const rampScaleWidth = rampScale.drawVertical(g, area.x + area.width, rampY, rampFullHeight, false, true);
            const rampX = area.x + yTitleWidth + yScaleWidth + graphAreaWidth + this.gap;
            const rampWidth = area.width - yTitleWidth - yScaleWidth - graphAreaWidth - rampScaleWidth - this.gap;
    ...
    get gap(): number { return this.scale * 3; }

    get colorMap(): ColorMap { return this.properties.getValue(PROP_COLOR_MAP); }
    get cropBottom(): number { return this.properties.getValue(PROP_CROP_BOTTOM); }
    get cropLeft(): number { return this.properties.getValue(PROP_CROP_LEFT); }
    get cropRight(): number { return this.properties.getValue(PROP_CROP_RIGHT); }
    get cropTop(): number { return this.properties.getValue(PROP_CROP_TOP); }
    ...
    get xAxisAxisColor(): Color { return this.properties.getValue(PROP_X_AXIS_AXIS_COLOR); }
    get xAxisAxisTitle(): string { return this.properties.getValue(PROP_X_AXIS_AXIS_TITLE); }
    get xAxisMajorTickStepHint(): number { return this.scale * this.properties.getValue(PROP_X_AXIS_MAJOR_TICK_STEP_HINT); }
    get xAxisMaximum(): number { return this.properties.getValue(PROP_X_AXIS_MAXIMUM); }
    get xAxisMinimum(): number { return this.properties.getValue(PROP_X_AXIS_MINIMUM); }
    get xAxisScaleFont(): Font { return this.properties.getValue(PROP_X_AXIS_SCALE_FONT).scale(this.scale); }
    get xAxisShowMinorTicks(): boolean { return this.properties.getValue(PROP_X_AXIS_SHOW_MINOR_TICKS); }
    get xAxisTitleFont(): Font { return this.properties.getValue(PROP_X_AXIS_TITLE_FONT).scale(this.scale); }
    get xAxisVisible(): boolean { return this.properties.getValue(PROP_X_AXIS_VISIBLE); }

    get yAxisAxisColor(): Color { return this.properties.getValue(PROP_Y_AXIS_AXIS_COLOR); }
    get yAxisAxisTitle(): string { return this.properties.getValue(PROP_Y_AXIS_AXIS_TITLE); }
    get yAxisMajorTickStepHint(): number { return this.scale * this.properties.getValue(PROP_Y_AXIS_MAJOR_TICK_STEP_HINT); }
    get yAxisMaximum(): number { return this.properties.getValue(PROP_Y_AXIS_MAXIMUM); }
    get yAxisMinimum(): number { return this.properties.getValue(PROP_Y_AXIS_MINIMUM); }
    get yAxisScaleFont(): Font { return this.properties.getValue(PROP_Y_AXIS_SCALE_FONT).scale(this.scale); }
    get yAxisShowMinorTicks(): boolean { return this.properties.getValue(PROP_Y_AXIS_SHOW_MINOR_TICKS); }
    get yAxisTitleFont(): Font { return this.properties.getValue(PROP_Y_AXIS_TITLE_FONT).scale(this.scale); }
    get yAxisVisible(): boolean { return this.properties.getValue(PROP_Y_AXIS_VISIBLE); }
}
