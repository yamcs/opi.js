...
export class Thermometer extends Widget {
  ...
  draw(g: Graphics) {
    ...
    const scaleArea: Bounds = {
      ...
      height:
        pipeArea.height - unitHeight + linearScale.calculateMargin(g, false),
    };
    ...
    if (this.showBulb) {
      ...
      const joint: Bounds = {
        x:
          Math.round(pipeArea.x + pipeArea.width / 2 - this.pipeWidth / 2) +
          this.outlineWidth,
  ...
  get showBulb(): boolean {
    return this.properties.getValue(PROP_SHOW_BULB);
  }
  ...
  get unit(): number {
    return this.properties.getValue(PROP_UNIT);
  }
