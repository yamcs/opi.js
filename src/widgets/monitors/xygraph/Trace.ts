...
export class Trace {
    ...
    init() {
        ...
        this.traceData = new TraceBuffer(bufferSize, plotMode,
            updateMode, concatenateData, chronological);
        if (this.xPV) {
    ...
    drawTrace(g: Graphics, points: Point[]) {
        ...
        if (this.traceType === 0) { // Solid Line
            g.strokePath({
                path: Path.fromPoints(points),
                color: this.traceColor,
                lineWidth,
            });
        } else if (this.traceType === 1) { // Dash Line
            g.strokePath({
                path: Path.fromPoints(points),
                color: this.traceColor,
                lineWidth,
        ...
        } else if (this.traceType === 2) { // Point
            // Nothing to do
        } else if (this.traceType === 3) { // Bar
        ...
        } else if (this.traceType === 4 || this.traceType === 5) { // Area, Line Area
            const yAxis = this.widget.getAxis(this.yAxisIndex).linearScale!;
            const originY = yAxis.getValuePosition(0);
        ...
        } else if (this.traceType === 6) { // Step Vertically
            for (let i = 1; i < points.length; i++) {
                const prev = points[i - 1];
                const point = points[i];
                g.strokePath({
        ...
        } else if (this.traceType === 7) { // Step Horizontally
            for (let i = 1; i < points.length; i++) {
                const prev = points[i - 1];
                const point = points[i];
                g.strokePath({
        ...
        } else if (this.traceType === 8) { // Dash Dot Line
            const { scale } = this;
            g.strokePath({
                path: Path.fromPoints(points),
                color: this.traceColor,
        ...
        } else if (this.traceType === 9) { // Dash Dot Dot Line
            const { scale } = this;
            g.strokePath({
                path: Path.fromPoints(points),
                color: this.traceColor,
        ...
        } else if (this.traceType === 10) { // Dot Line
            const { scale } = this;
            g.strokePath({
                path: Path.fromPoints(points),
                color: this.traceColor,
    ...
    get antiAlias(): boolean { return this.getValue('anti_alias'); }
    get bufferSize(): number { return this.getValue('buffer_size'); }
    get concatenateData(): boolean { return this.getValue('concatenate_data'); }
    get lineWidth(): number { return this.scale * this.getValue('line_width'); }
    get name(): string { return this.getValue('name'); }
    get plotMode(): number { return this.getValue('plot_mode'); }
    get pointSize(): number { return this.scale * this.getValue('point_size'); }
    get pointStyle(): number { return this.getValue('point_style'); }
    get traceColor(): Color { return this.getValue('trace_color'); }
    get traceType(): number { return this.getValue('trace_type'); }
    get updateDelay(): number { return this.getValue('update_delay'); }
    get updateMode(): number { return this.getValue('update_mode'); }
    get visible(): boolean { return this.getValue('visible'); }
    get xAxisIndex(): number { return this.getValue('x_axis_index'); }
    get xPV(): string { return this.getValue('x_pv'); }
    get yAxisIndex(): number { return this.getValue('y_axis_index'); }
    get yPV(): string { return this.getValue('y_pv'); }
}
