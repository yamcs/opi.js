...
export class XYGraphToolbar {
    ...
    constructor(readonly xyGraph: XYGraph) {
        this.showLegend = this.createToggleButton('ShowLegend.png', 'Show Legend', () => {
        ...
        this.autoScale = this.createButton('AutoScale.png', 'AutoScale.png', 'Perform Auto Scale', () => {
            const autoScaleCommand = new ZoomCommand(xyGraph.getXAxes(), xyGraph.getYAxes());
            xyGraph.performAutoScale();
            autoScaleCommand.saveState();
            this.addCommand(autoScaleCommand);
        });
        this.rubberbandZoom = this.createToolButton('RubberbandZoom.png', 'Rubberband Zoom', () => {
        ...
        this.horizontalZoom = this.createToolButton('HorizontalZoom.png', 'Horizontal Zoom', () => {
        ...
        this.verticalZoom = this.createToolButton('VerticalZoom.png', 'Vertical Zoom', () => {
        ...
        this.camera = this.createButton('camera.png', 'camera.png', 'Save Snapshot to PNG File', () => {
        ...
        const loadPromises = this.buttons.map(button => button.loadPromise)
    ...
    toggleTool(button: Button) {
        ...
        if (wasPushed) { // Restore
            button.pushed = true;
        } else { // Default to Mouse Arrow
            this.mouseArrow.pushed = true;
        }
        this.xyGraph.requestRepaint();
    }

    private createToolButton(imageFile: string, tooltip: string, action: () => void) {
        const button = this.createToggleButton(imageFile, tooltip, action);
        button.toolButton = true;
        return button;
    }

    private createToggleButton(imageFile: string, tooltip: string, action: () => void) {
    ...
    private createButton(
        ...
        const button = new Button(this, imageFile, disabledImageFile, tooltip, action);
    ...
    private drawButton(g: Graphics, x: number, y: number, button: Button) {
        ...
        g.fillRect({ x, y, width: buttonSize, height: buttonSize, color: BUTTON_COLOR });
        ...
        if (this.imagesLoaded) {
            const imageEl = button.disabled ? button.disabledImageElement : button.imageElement;
    ...
    get scale() { return this.xyGraph.scale; }

    get buttonSize() { return 25 * this.scale; }
}
